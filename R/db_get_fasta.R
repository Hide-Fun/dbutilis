...
#' @export
db_get_fasta <- function(.acc, .dest_dir, .n_max, .sleep, .try_max = NULL) {
  acc <- .acc
  ...
  path_list <- unlist(purrr::map(1:length(use_urls), ~ paste0(.dest_dir, "/", .x, ".fasta")))
  # download.
  ...
  if (!is.null(.try_max)) {
    ...
    for (i in 1:.try_max) {
      if (length(which(res_list[[i]] == "fault")) != 0) {
        ...
        res_list[[i + 1]] <- purrr::map2_chr(use_urls, path_list, ~ curl_download2(.x, .y, .sleep = .sleep))
        res <- res_list[[i + 1]]
      } else {
        res <- res_list[[i]]
        use_urls <- use_urls[which(res_list[[i]] == "fault")]
